...
; !function (window, undefined) {
    ...
    Class.pt.auto = function (index) {
        ...
        switch (config.type) {
            case 2:
                setHeight('iframe');
                //20170411星期二by乐章最少宽度
                if (config.minWidth > layero.outerWidth()) {
                    layero.width(config.minWidth);
                }
                break;
            default:
                if (config.area[1] === '') {
    ...
    layer.iframeAuto = function (index) {
        if (!index) return;
        //宽20170410星期一by乐章
        var wid = layer.getChildFrame('html', index)[0].scrollWidth+20;
        var bodyWW = document.documentElement.clientWidth;
        var layW = wid > bodyWW ? bodyWW : wid;
        //高
        ...
        var titHeight = layero.find(doms[1]).outerHeight() || 0;
        var btnHeight = layero.find('.' + doms[6]).outerHeight() || 0;
        var layH = heg + titHeight + btnHeight
        layH = layH > bodyHH ? bodyHH : layH;
        //处理top20170410星期一by乐章
        var layTop = (bodyHH - layH) * 0.5;
        var layLeft = (bodyWW - layW) * 0.5
        layero.css({
            width: layW,
            height: layH,
            top: layTop,
